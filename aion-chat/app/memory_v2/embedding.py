...
def _proxy_value(provider: str, raw: dict) -> str:
    ...
    if not value:
        if provider == "gemini":
            value = _env("AION_GEMINI_PROXY") or _env("AION_PROVIDER_PROXY")
        else:
            value = _env("AION_OPENAI_PROXY") or _env("AION_PROVIDER_PROXY")
...
def load_embedding_config(overrides: dict | None = None) -> dict[str, Any]:
    ...
    provider = (_env("AION_MEMORY_EMBEDDING_PROVIDER") or raw.get("provider") or "siliconflow")
    ...
    model = _env("AION_MEMORY_EMBEDDING_MODEL") or raw.get("model") or defaults["model"]
    dimensions_raw = _env("AION_MEMORY_EMBEDDING_DIMENSIONS") or raw.get("dimensions")
