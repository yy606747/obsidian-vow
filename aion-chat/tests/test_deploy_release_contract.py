...
def test_build_and_hot_copy_install_the_same_hash_locked_runtime():
    dockerfile = (ROOT / "aion-chat/Dockerfile").read_text()
...
def test_container_checks_do_not_depend_on_unpinned_system_packages():
    for relative in ("aion-chat/docker-compose.yml", "deploy/docker-compose.prod.yml"):
    ...
    assert "AION_RELEASE_IMAGE" in production["services"]["app"]["image"]


def test_installed_dependency_versions_match_release_locks():
    for relative in ("aion-chat/requirements.lock", "aion-chat/requirements-test.lock"):
