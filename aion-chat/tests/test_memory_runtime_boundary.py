...
AION_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = AION_ROOT.parent
RUNTIME_PACKAGES = (AION_ROOT / "app/memory_v2", AION_ROOT / "app/memory_v3")
        ...
        for path in package.rglob("*.py"):
            for name in _import_names(path):
                if name == "devtools" or name.startswith("devtools."):
                    violations.append(f"{path.relative_to(AION_ROOT)} -> {name}")
    assert violations == []
...
def test_memory_v2_runtime_contains_no_experiment_modules() -> None:
    present = {
        path.name
        for path in (AION_ROOT / "app/memory_v2").glob("*.py")
...
def test_devtools_and_tests_are_excluded_from_production_context() -> None:
    ...
    assert "aion-chat/devtools/" in entries
    assert "aion-chat/tests/" in entries
