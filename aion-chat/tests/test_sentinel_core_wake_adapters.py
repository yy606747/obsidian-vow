...
def test_legacy_core_wake_ports_write_db_broadcast_stream_and_monitor_log(tmp_path):
    ...
    async def run_flow():
        ...
        await ports.broadcast_monitor_alert("Aion的哨兵唤醒了主脑")
        system_message = await ports.insert_system_wake_notice(
            conv_id="conv_sentinel",
            content="Aion的哨兵唤醒了主脑",
    ...
    assert deps.broadcasts[0]["data"]["content"] == "Aion的哨兵唤醒了主脑"
    ...
    assert rows == [
        {"role": "system", "content": "Aion的哨兵唤醒了主脑", "attachments": "[]"},
