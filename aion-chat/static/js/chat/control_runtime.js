(function (global) {
  ...
  const OWNER_KEY = "aion_control_owner_client_id";
  const TIDE_OWNER_KEY = "aion_tide_owner_client_id";
  const OWNER_LOCK_PREFIX = "aion_control_owner_lock:";
  const SAFETY_KEY_PREFIX = "aion_control_safety_stop:";
  const CHANNEL_NAME = "aion_control_runtime";
  ...
  function _emit(reason) {
    ...
    try {
      global.dispatchEvent(new CustomEvent("aion:control-session", { detail }));
    } catch (e) {}
    try {
  ...
  function _snapshot(kind) {
    ...
    if (kind === "tide") {
      return normalizeSnapshot("tide", { tide_mode: !!global.tideMode, toy_connected: !!global.AionMuse?.isSupported?.() });
    }
  ...
  function _releaseLocalMode(kind, reason) {
    ...
    if (kind === "tide") {
      try { global.AionMuse?.stop?.(); } catch (e) {}
