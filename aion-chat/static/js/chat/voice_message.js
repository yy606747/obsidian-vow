...
const voiceMessageRecorder = {
  ...
  async start() {
    ...
    try {
      if (window.AionAudio) {
        ...
        try { alreadyRecording = !!window.AionAudio.isRecording(); } catch (_) {}
        let nativeReady = alreadyRecording;
        if (!nativeReady) {
          try { nativeReady = !!window.AionAudio.start(); } catch (_) {}
  ...
  _cleanupCapture() {
    if (this.ownsNative && window.AionAudio) {
      try { window.AionAudio.stop(); } catch (_) {}
    }
    this.ownsNative = false;
    this.useNative = false;
    if (this.processor) {
