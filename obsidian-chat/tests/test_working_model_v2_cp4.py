...
def test_monitor_closes_flag_at_cap_and_keeps_trigger_until_cleanup(
    ...
    prereg = _approved_natural_prereg(
        ...
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/test",
    )
...
def test_monitor_automatically_closes_flag_at_deadline(
    ...
    prereg = _approved_natural_prereg(
        preregistration_path,
        behavior_path=behavior_path,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/deadline-test",
    )
    monkeypatch.setattr(shadow, "_validate_preregistration", lambda *_a, **_k: prereg)
    monkeypatch.setattr(shadow, "_resolve_private_path", lambda *_a, **_k: private_dir)
    shadow.activate(
...
def test_runtime_contract_rejects_hostname_substring_spoof(monkeypatch) -> None:
    ...
    monkeypatch.delenv("OBSIDIAN_PROVIDER_TIMEOUT_SEC", raising=False)
    monkeypatch.setattr(
        offline,
        "get_slot",
        lambda _name: {
...
def test_pending_authorization_blocks_both_paid_entries_before_mutation(
    ...
    common = {
        ...
        "private_run_dir": "obsidian-chat/data/working_model_v2_cp4/pending-test",
    }
    natural_prereg = {
        **common,
        "schema_version": shadow.PREREGISTRATION_SCHEMA_VERSION,
...
def test_offline_contract_rejects_natural_coupling_and_budget_drift(
    ...
    prereg = _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/offline-contract-test",
        identity=identity,
    )
...
def test_offline_runtime_rejects_writer_outside_explicit_allowlist(
    ...
    prereg = _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/allowlist-test",
        identity=identity,
    )
...
def test_offline_preflight_is_not_ok_when_source_identity_drifted(
    ...
    _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/preflight-drift-test",
...
def test_offline_cli_monetary_cap_must_match_before_any_mutation(
    ...
    _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/cost-mismatch-test",
        identity=identity,
    )
    monkeypatch.setattr(
        offline.cp4,
...
def test_offline_monetary_guard_is_void_not_provider_failed(
    ...
    _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/cost-void-test",
        identity=identity,
...
def test_offline_call_count_guard_is_void_not_provider_failed(
    ...
    prereg = _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/call-void-test",
        identity=identity,
    )
...
def test_runtime_contract_freezes_effective_gate_and_writer_transport(
    ...
    prereg = _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/transport-test",
        identity=identity,
    )
    monkeypatch.setenv("OBSIDIAN_OPENAI_PROXY", "http://127.0.0.1:7890")
...
def test_prepare_source_snapshot_copies_roots_without_mutating_inputs(
    ...
    prereg = {
        ...
        "source_snapshot": {"sealed": False},
        "private_run_dir": "obsidian-chat/data/working_model_v2_cp4/test-source",
    }
    monkeypatch.setattr(
        offline,
...
def test_offline_runner_stops_after_first_technical_failure_and_cannot_resume(
    ...
    prereg = _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/fail-fast-test",
        identity=identity,
    )
...
def test_offline_runner_uses_twenty_fake_calls_and_never_writes_source(
    ...
    prereg = _approved_offline_prereg(
        preregistration_path,
        source_db=source_db,
        private_run_dir="obsidian-chat/data/working_model_v2_cp4/test-offline",
        identity=identity,
    )
    inputs = json.loads(OFFLINE_INPUTS.read_text(encoding="utf-8"))
