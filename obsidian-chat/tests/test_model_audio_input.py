...
def test_frontend_voice_message_contract_is_lazy_and_fail_closed():
    ...
    bridge = (
        ROOT.parent
        / "ObsidianApp/app/src/main/java/app/obsidianvow/core/AudioBridge.java"
    ).read_text(encoding="utf-8")

    assert 'id="voiceHoldBtn"' in html
    assert '/static/js/chat/voice_message.js?v=20260905-brand' not in html
    ...
    assert "window.ObsidianAudio.start()" in recorder
