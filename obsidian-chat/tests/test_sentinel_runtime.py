...
def _patch_runtime_env(monkeypatch):
    ...
    monkeypatch.setattr(sentinel_runtime, "load_worldbook", lambda: {"user_name": "用户", "ai_name": "Arden"})
...
def test_call_core_prompt_includes_persona_temperature_and_live_toy_capability(monkeypatch, tmp_path):
    ...
    assert messages[0] == {"role": "user", "content": "[关于你自己：Arden]\nArden 是温柔但有掌控感的伴侣。"}
    assert messages[1]["content"] == "（嗯，我知道自己是Arden。）"
