...
def test_settings_migration_preserves_a_custom_gate_slot(monkeypatch):
    monkeypatch.delenv("OBSIDIAN_SILICONFLOW_KEY", raising=False)
    data = {
...
def test_siliconflow_endpoint_can_use_legacy_credential_without_copying_it(monkeypatch):
    ...
    monkeypatch.delenv("OBSIDIAN_ENDPOINT_SF_KEY", raising=False)
    monkeypatch.delenv("OBSIDIAN_SILICONFLOW_KEY", raising=False)
    monkeypatch.setattr(
        config,
        "SETTINGS",
...
def test_slot_chat_uses_only_the_explicit_obsidian_proxy(monkeypatch):
    client_kwargs = {}
    ...
    monkeypatch.setenv("OBSIDIAN_OPENAI_PROXY", "http://explicit.invalid:7890")
    monkeypatch.setattr(
        ai_providers,
        "get_slot",
        lambda _name: {
