...
def test_source_snapshot_includes_untracked_code_but_not_data_keys_or_builds(tmp_path):
    root = tmp_path / "repo"
    for name in (
        "obsidian-chat/config.py", "obsidian-chat/app/new_feature.py", "obsidian-chat/data/chat.db",
        "obsidian-chat/.env", "obsidian-chat/wheels/unused.whl", "ObsidianApp/app/build/generated.txt",
    ...
    assert set(manifest["files"]) == {
        "source/obsidian-chat/config.py", "source/obsidian-chat/app/new_feature.py",
    ...
    assert (tmp_path / "restored/obsidian-chat/app/new_feature.py").read_text() == "sample"
...
def test_data_backup_restores_live_wal_databases_uploads_and_configuration(tmp_path):
    ...
    env_file.write_text("OBSIDIAN_AUTH_TOKEN=test-only", encoding="utf-8")
    connections = []
    try:
        ...
        assert (target / "env/0-.env").read_text() == "OBSIDIAN_AUTH_TOKEN=test-only"
