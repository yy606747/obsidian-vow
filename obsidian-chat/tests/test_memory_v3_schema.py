...
def test_preflight_is_read_only_and_separates_config_layers(tmp_path):
    ...
    (repo_root / "obsidian-chat").mkdir(parents=True)
    (repo_root / "deploy").mkdir()
    (repo_root / "obsidian-chat/main.py").write_text("uvicorn.run('main:app')", encoding="utf-8")
    (repo_root / "obsidian-chat/Dockerfile").write_text(
