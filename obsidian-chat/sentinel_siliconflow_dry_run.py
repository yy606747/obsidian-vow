...
def resolve_siliconflow_endpoint(
    *,
    ...
    if not endpoint.get("api_key"):
        raise ValueError("missing SiliconFlow API key: set OBSIDIAN_SILICONFLOW_KEY or data/settings.json siliconflow_key")
...
def _resolve_siliconflow_key() -> tuple[str, str]:
    env_key = os.environ.get("OBSIDIAN_SILICONFLOW_KEY", "").strip()
    if env_key:
        return env_key, "env:OBSIDIAN_SILICONFLOW_KEY"
