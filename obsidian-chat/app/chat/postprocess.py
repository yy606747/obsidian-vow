...
from brand_compat import LEGACY_STRUCTURED_FENCE
...
def _maybe_json_object(text: str) -> dict | None:
    ...
    if raw.startswith("```"):
        ...
        if lines and lines[0].strip().lower() in {"```", "```json", "```obsidian", LEGACY_STRUCTURED_FENCE} and lines[-1].strip() == "```":
...
def looks_like_structured_reply(text: str) -> bool:
    ...
    return raw.startswith(("{", "```json", "```obsidian", LEGACY_STRUCTURED_FENCE))
