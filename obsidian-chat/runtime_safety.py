...
TEST_MODE = os.environ.get("OBSIDIAN_TEST_MODE", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def resolve_data_dir(base_dir: Path) -> Path:
    ...
    configured = os.environ.get("OBSIDIAN_DATA_DIR", "").strip()
    if TEST_MODE and not configured:
        raise RuntimeError("测试模式必须显式设置 OBSIDIAN_DATA_DIR，不能使用真实数据目录。")
...
def install_test_network_guard() -> None:
    ...
    def blocked(*_args, **_kwargs):
        raise RuntimeError("OBSIDIAN_TEST_MODE 禁止真实网络请求，请使用模拟传输。")
