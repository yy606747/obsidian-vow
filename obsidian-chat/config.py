...
# 查找顺序：项目根 .env → obsidian-chat/.env。有 python-dotenv 就用，没有就手工读。
...
def _ensure_endpoints_and_slots(data: dict) -> bool:
    ...
    if siliconflow_endpoint is None and (
        data.get("siliconflow_key") or _env("OBSIDIAN_SILICONFLOW_KEY")
    ...
    gemini_key_available = bool(
        data.get("gemini_key") or _env("OBSIDIAN_GEMINI_KEY")
    )
...
def get_endpoint(endpoint_id: str) -> dict | None:
    """按 id 查端点配置。环境变量 OBSIDIAN_ENDPOINT_{ID}_KEY 会覆盖 api_key。"""
    for ep in SETTINGS.get("endpoints", []):
        if ep.get("id") == endpoint_id:
            env_key = _env(f"OBSIDIAN_ENDPOINT_{str(endpoint_id).upper()}_KEY")
            if not env_key and (
                endpoint_id == "sf"
                or "api.siliconflow.cn" in str(ep.get("base_url") or "")
            ):
                env_key = _env("OBSIDIAN_SILICONFLOW_KEY") or SETTINGS.get(
                    "siliconflow_key", ""
                )
            if not env_key and ep.get("type") == "gemini":
                env_key = _env("OBSIDIAN_GEMINI_KEY") or SETTINGS.get(
                    "gemini_key", ""
                )
            if env_key:
                ep = dict(ep)
...
def get_key(provider: str) -> str:
    """优先读取 OBSIDIAN_* 环境变量；回退到 settings.json。"""
    if provider == "gemini":
        return _env("OBSIDIAN_GEMINI_KEY") or SETTINGS.get("gemini_key", "")
    if provider == "gemini_free":
        return (_env("OBSIDIAN_GEMINI_FREE_KEY")
                or SETTINGS.get("gemini_free_key", "")
                or _env("OBSIDIAN_GEMINI_KEY")
                or SETTINGS.get("gemini_key", ""))
    if provider == "aipro":
        return _env("OBSIDIAN_AIPRO_KEY") or SETTINGS.get("aipro_key", "")
    return _env("OBSIDIAN_SILICONFLOW_KEY") or SETTINGS.get("siliconflow_key", "")
