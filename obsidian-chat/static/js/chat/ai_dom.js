...
// AI Dom 模式状态
let aiDomMode = false;
let aiDomSafeword = localStorage.getItem('obsidian_safeword') || '';
let aiDomCncEnabled = localStorage.getItem('obsidian_cnc_enabled') === '1';
let aiDomCncWeakness = (() => {
  try { const raw = localStorage.getItem('obsidian_cnc_weakness') || ''; return raw.split('\n').map(s=>s.trim()).filter(Boolean); }
  catch(e) { return []; }
})();
...
async function soConnect() {
  ...
  if (window.ObsidianBle) {
    if (window.ObsidianBle.configure) window.ObsidianBle.configure(JSON.stringify(d));
    window.ObsidianBle.connect();
    return;
  }
...
async function soDisconnect() {
  if (window.ObsidianBle && window.ObsidianBle.isConnected()) window.ObsidianBle.disconnect();
...
async function cxSend(bytes) {
  if (window.ObsidianBle && window.ObsidianBle.isConnected()) {
    const hex = bytes.map(b => b.toString(16).padStart(2, '0')).join('');
    try {
      window.ObsidianBle.sendData(hex);
      return true;
    } catch (e) {
...
// ── 失控系列（sk30/sk40）：通过 ObsidianAdv 广播桥控制 ──
let skConnected = false;

async function skConnect(devKey) {
  if (!window.ObsidianAdv) { aiDomLog('需要 Obsidian Vow Android App（广播桥不支持浏览器）', 'err'); throw new Error('no_adv'); }
  if (!window.ObsidianAdv.isSupported()) { aiDomLog('此设备不支持 BLE 广播', 'err'); throw new Error('no_adv_hw'); }
  const advDev = AI_DOM_DEVICES[devKey].advDevice;
  window.ObsidianAdv.configure(JSON.stringify({ device: advDev }));
  skConnected = true;
...
async function skDisconnect() {
  if (window.ObsidianAdv) window.ObsidianAdv.disconnect();
  skConnected = false;
}

async function skPlay(payload) {
  if (!window.ObsidianAdv) return false;
  const vv = Math.round(Math.max(0, Math.min(10, (payload && payload.v) || 0)));
  const ss = Math.round(Math.max(0, Math.min(10, (payload && payload.s) || 0)));
  if (vv === 0 && ss === 0) return skStop();
  window.ObsidianAdv.play(JSON.stringify({ v: vv, s: ss }));
  return true;
}

async function skStop() {
  if (!window.ObsidianAdv) return false;
  window.ObsidianAdv.stop();
  return true;
}
...
const toyDriver = {
  ...
  isConnected() {
    ...
    if (this.active === 'sosexy') return !!(toyConnected || (window.ObsidianBle && window.ObsidianBle.isConnected()));
    if (this.active === 'cx492b') return !!(cxConnected  || (window.ObsidianBle && window.ObsidianBle.isConnected()));
    return false;
  },
  async connect(id) {
