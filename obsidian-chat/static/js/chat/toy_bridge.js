...
function toyBridgeMetadata(extra) {
  ...
  return Object.assign({
    page: 'chat',
    transport: window.ObsidianBle ? 'android_ble' : 'web_bluetooth',
...
function toyBridgeIsConnected() {
  ...
  try { if (window.ObsidianBle && typeof window.ObsidianBle.isConnected === 'function' && window.ObsidianBle.isConnected()) return true; } catch(e) {}
  return false;
}
setInterval(() => {
...
async function toySendData2(hexCmd) {
  if (window.ObsidianBle && window.ObsidianBle.isConnected()) {
    ...
    try {
      window.ObsidianBle.sendData(hexCmd);
      return true;
    } catch(e) {
