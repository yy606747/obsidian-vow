...
def _resolve_timeout(endpoint: dict | None, default: float = 120.0) -> float:
    ...
    if raw in (None, ""):
        raw = _env("OBSIDIAN_PROVIDER_TIMEOUT_SEC")
    try:
        value = float(raw)
...
def _resolve_proxy(provider_type: str, endpoint: dict | None,
    ...
    if provider_type in ("gemini", "vertex"):
        env_raw = _env("OBSIDIAN_GEMINI_PROXY", "OBSIDIAN_PROVIDER_PROXY")
        if env_raw:
            return _normalize_proxy_value(env_raw)
        return None
    env_raw = _env("OBSIDIAN_OPENAI_PROXY", "OBSIDIAN_PROVIDER_PROXY")
    if env_raw:
        return _normalize_proxy_value(env_raw)
    return None
