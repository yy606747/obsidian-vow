...
from brand_compat import LEGACY_AUTH_COOKIE
...
# OBSIDIAN_AUTH_TOKEN 为空 → 完全关闭鉴权（保持本地开发零配置）
# 否则全站要求 token，以下三种任一通过：
#   1. Cookie obsidian_token=<TOKEN>
#   2. Header Authorization: Bearer <TOKEN>
#   3. Query ?token=<TOKEN>（首次访问后会写 cookie 并跳转到去掉 token 的 URL）
_AUTH_TOKEN = (os.environ.get("OBSIDIAN_AUTH_TOKEN") or "").strip()
_AUTH_COOKIE = "obsidian_token"
...
def _check_token(request: Request) -> bool:
    for name in (_AUTH_COOKIE, LEGACY_AUTH_COOKIE):
...
async def auth_middleware(request: Request, call_next):
    ...
    if _check_token(request):
        ...
        legacy_cookie = request.cookies.get(LEGACY_AUTH_COOKIE, "")
        if legacy_cookie and _safe_eq(legacy_cookie, _AUTH_TOKEN) and not _safe_eq(
            request.cookies.get(_AUTH_COOKIE, ""), _AUTH_TOKEN,
        ):
...
async def websocket_endpoint(ws: WebSocket):
    if _AUTH_TOKEN:
        ...
        ok = _check_token(ws) or (qtoken and _safe_eq(qtoken, _AUTH_TOKEN))
        if not ok:
...
if __name__ == "__main__":
    ...
    bind_host = os.environ.get("OBSIDIAN_BIND_HOST", "0.0.0.0")
    bind_port = int(os.environ.get("OBSIDIAN_BIND_PORT", "18080"))
